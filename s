...
const PoolTable: React.FC = () => {
  const [pools, setPools] = useState<Pool[]>([hardcodedPool]); // Initialize with hardcoded pool
  ...
  useEffect(() => {
    ...
    getFilteredPoolInfo()
      .then(response => {
        setPools([hardcodedPool, ...response.data]); // Add hardcoded pool to fetched data
        setIsLoading(false);
      })
      .catch(err => {
  ...
  const handleMigrate = () => {
    console.log('Migrate selected pools:', selectedPools);
  };
  ...
  return (
    <div className="container">
      ...
      {showMigrate && (
        <div className="dialogStyles.migrationSection">
